...
def prepare_list_of_dict(
    data: List, item_name: str, is_reverse: bool = False
) -> List[Dict]:
...
async def main() -> None:
    ...
    nhy = [href + [y_gr] for y_gr, href in zip(y_growths, hrefs)]

    stock_urls = [stock_url for _, stock_url, _ in nhy]
    ...
    price_data = prepare_list_of_dict(data, "price", True)
    pe_data = prepare_list_of_dict(data, "P/E")
    growth_data = prepare_list_of_dict(data, "growth", True)
    profit_data = prepare_list_of_dict(data, "potential profit", True)

    filenames = ["price", "pe", "growth", "profit"]
    all_data = [price_data, pe_data, growth_data, profit_data]
    write_files = [
